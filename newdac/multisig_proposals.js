...
class MultisigProposalsHandler {
    ...
    async thresholdFromName(name, dac_id){
        ...
        return new Promise(async (resolve, reject) => {
            ...
            const scope = custodian_contract;
            ...
            switch (name) {
                ...
                case 'med':
                    ...
                    break;
                case 'one':
                    resolve(dac_config.auth_threshold_one);
                    break;
                case 'high':
                case 'active':
    ...
    async permissionToThreshold(perm, dac_id) {
        return new Promise(async (resolve) => {
            const self = this;
           const auth_account = 'dacauth11111';
	...
	let res_approvals = await eosTableAtBlock(table_query);
		...
		//this.logger.info('here is after reqested_approvals', approvals_data.requested_approvals);    
                approvals_data.provided_approvals = approvals_data.provided_approvals.map((p) => {
		    return p.level;
                });
                //this.logger.info('here is after provided_approvals', approvals_data.provided_approvals);	
            }
        }
       ...
       // output.provided_approvals = approvals_data.provided_approvals;
        ...
        if (!res_data.results.length){
            ...
            res_data = await eosTableAtBlock({
                db,
                code: {$in:msig_contracts},
                scope: this.dac_directory._custodian_contracts.get(dac_id),
                table: 'proposals',
                block_num: check_block,
                data_query: local_data_query
            });
